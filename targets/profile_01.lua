local obf_stringchar = string.char;
local obf_stringbyte = string.byte;
local obf_stringsub = string.sub;
local obf_bitlib = bit32 or bit;
local obf_XOR = obf_bitlib.bxor;
local obf_tableconcat = table.concat;
local obf_tableinsert = table.insert;
local function LUAOBFUSACTOR_DECRYPT_STR_0(LUAOBFUSACTOR_STR, LUAOBFUSACTOR_KEY)
	local result = {};
	for i = 1, #LUAOBFUSACTOR_STR do
		obf_tableinsert(result, obf_stringchar(obf_XOR(obf_stringbyte(obf_stringsub(LUAOBFUSACTOR_STR, i, i + 1)), obf_stringbyte(obf_stringsub(LUAOBFUSACTOR_KEY, 1 + (i % #LUAOBFUSACTOR_KEY), 1 + (i % #LUAOBFUSACTOR_KEY) + 1))) % 256));
	end
	return obf_tableconcat(result);
end
print(LUAOBFUSACTOR_DECRYPT_STR_0("\130\0\44\128\186\106\241\3\38\150\182\104\165\80\41\139\190\124\180\20", "\24\209\112\69\228\223"));
local v0 = game:GetService(LUAOBFUSACTOR_DECRYPT_STR_0("\191\30\211\15\155\1\192\7\141", "\100\232\113\161"));
local v1 = game:GetService(LUAOBFUSACTOR_DECRYPT_STR_0("\47\11\210\175\121\221\243", "\180\127\103\179\214\28\175\128"));
local v2 = game:GetService(LUAOBFUSACTOR_DECRYPT_STR_0("\109\83\234\114\221\221\186\12\92\67", "\101\63\38\132\33\184\175\204"));
local v3 = game:GetService(LUAOBFUSACTOR_DECRYPT_STR_0("\113\165\238\60\45\73\220\4\80\133\238\60\18\78\207\20", "\113\36\214\139\78\100\39\172"));
local v4 = game:GetService(LUAOBFUSACTOR_DECRYPT_STR_0("\1\36\168\125\210\6\54\191\110\213\54\54", "\188\85\83\205\24"));
local v5 = game:GetService(LUAOBFUSACTOR_DECRYPT_STR_0("\96\52\225\205\217\168\112\135", "\224\44\93\134\165\173\193\30"));
local v6 = game:GetService(LUAOBFUSACTOR_DECRYPT_STR_0("\247\203\40\218\167\87\6\219\207\210\47\218\159\87\4\243\198\199\40", "\146\161\162\90\174\210\54\106"));
local v7 = v1['LocalPlayer'];
local v8 = v7['Character'] or v7['CharacterAdded']:Wait();
local v9 = v8:WaitForChild(LUAOBFUSACTOR_DECRYPT_STR_0("\88\248\77\89\46\65\121\233\114\87\47\90\64\236\82\76", "\46\16\141\32\56\64"));
local v10 = v8:WaitForChild(LUAOBFUSACTOR_DECRYPT_STR_0("\114\193\141\253\84\219\137\248", "\156\58\180\224"));
local v11 = v0['CurrentCamera'];
local v12 = {};
local v13 = false;
local v14 = true;
local v15 = true;
local v16 = false;
local v17 = false;
local v18 = false;
local v19 = true;
local v20 = 279 - (100 + 163);
local v21 = nil;
local v22 = v20;
local v23 = 0 - 0;
local v24 = 1228 - (322 + 905);
local v25 = 661 - (602 + 9);
local v26 = nil;
local v27 = nil;
local v28 = v25;
local v29 = 1192 - (449 + 740);
local v30 = -v29;
local v31;
local function v32(v58)
	if v12[v58] then
		local v118 = 872 - (826 + 46);
		while true do
			if (v118 == (947 - (245 + 702))) then
				v12[v58]:Disconnect();
				v12[v58] = nil;
				break;
			end
		end
	end
end
local function v33()
	local v59 = 0 - 0;
	while true do
		if (v59 == (0 + 0)) then
			if not v10 then
				return;
			end
			if v17 then
				v10['WalkSpeed'] = v22;
			else
				v10['WalkSpeed'] = v21 or v20;
			end
			break;
		end
	end
end
local function v34()
	local v60 = 1898 - (260 + 1638);
	while true do
		if (v60 == (440 - (382 + 58))) then
			if not v10 then
				return;
			end
			v32(LUAOBFUSACTOR_DECRYPT_STR_0("\210\87\86\195\214\70\95\205\225\117\82\201\235\81\95\204", "\168\133\54\58"));
			v60 = 3 - 2;
		end
		if (v60 == (1 + 0)) then
			v12[LUAOBFUSACTOR_DECRYPT_STR_0("\60\88\90\64\206\147\14\92\82\104\245\130\5\94\83\79", "\227\107\57\54\43\157")] = v10:GetPropertyChangedSignal(LUAOBFUSACTOR_DECRYPT_STR_0("\176\251\195\141\180\234\202\131\131", "\230\231\154\175")):Connect(function()
				local v125 = 0 - 0;
				local v126;
				while true do
					if (v125 == (0 - 0)) then
						if not v17 then
							return;
						end
						v126 = v10['WalkSpeed'];
						v125 = 1206 - (902 + 303);
					end
					if (v125 == (1 - 0)) then
						if (v126 ~= v22) then
							local v148 = 0 - 0;
							local v149;
							while true do
								if ((1 + 0) == v148) then
									if ((v149 - v23) >= v24) then
										local v152 = 1690 - (1121 + 569);
										while true do
											if (v152 == (214 - (22 + 192))) then
												v23 = v149;
												print(LUAOBFUSACTOR_DECRYPT_STR_0("\42\194\184\208\197\75\155\20\240\189\156\237\116\138\28\229\132", "\235\113\149\217\188\174\24"), LUAOBFUSACTOR_DECRYPT_STR_0("\166\77\238\124\239\128\88\247\124\162\145\88\230\125\245", "\207\225\44\131\25"), v126, "→ Forced:", v22);
												break;
											end
										end
									end
									break;
								end
								if (v148 == (683 - (483 + 200))) then
									v10['WalkSpeed'] = v22;
									v149 = os.clock();
									v148 = 1464 - (1404 + 59);
								end
							end
						end
						break;
					end
				end
			end);
			break;
		end
	end
end
local function v35()
	local v61 = 0 - 0;
	while true do
		if (v61 == (0 - 0)) then
			if not v10 then
				return;
			end
			if v18 then
				if v10['UseJumpPower'] then
					v10['JumpPower'] = v28;
				else
					v10['JumpHeight'] = v28 / (772 - (468 + 297));
				end
			elseif v10['UseJumpPower'] then
				v10['JumpPower'] = v26 or v25;
			else
				v10['JumpHeight'] = v27 or (569 - (334 + 228));
			end
			break;
		end
	end
end
local function v36(v62)
	local v63 = 0 - 0;
	while true do
		if (v63 == (4 - 2)) then
			v26 = v10['JumpPower'];
			v27 = v10['JumpHeight'];
			v63 = 5 - 2;
		end
		if ((1 + 0) == v63) then
			v10 = v62:WaitForChild(LUAOBFUSACTOR_DECRYPT_STR_0("\99\198\181\77\21\114\66\215", "\29\43\179\216\44\123"));
			v21 = v10['WalkSpeed'];
			v63 = 238 - (141 + 95);
		end
		if (v63 == (0 + 0)) then
			v8 = v62;
			v9 = v62:WaitForChild(LUAOBFUSACTOR_DECRYPT_STR_0("\149\204\45\77\179\214\41\72\143\214\47\88\141\216\50\88", "\44\221\185\64"));
			v63 = 2 - 1;
		end
		if (v63 == (9 - 5)) then
			v35();
			break;
		end
		if (v63 == (1 + 2)) then
			v33();
			v34();
			v63 = 10 - 6;
		end
	end
end
local function v37(v64, v65, v66)
	if (v14 and v31 and v31['Notify']) then
		v31:Notify(v64, v65, v66 or (3 + 0));
	end
end
local function v38(...)
	if v13 then
		print(...);
	end
end
if v7['Character'] then
	v36(v7.Character);
end
v32(LUAOBFUSACTOR_DECRYPT_STR_0("\34\239\73\77\114\2\243\77\77\82\5\227\77\91", "\19\97\135\40\63"));
v12[LUAOBFUSACTOR_DECRYPT_STR_0("\141\84\50\41\46\50\186\89\33\26\43\53\171\88", "\81\206\60\83\91\79")] = v7['CharacterAdded']:Connect(function(v67)
	local v68 = 0 + 0;
	while true do
		if (v68 == (0 - 0)) then
			v36(v67);
			v38(LUAOBFUSACTOR_DECRYPT_STR_0("\109\163\209\96\46\192\89\161\92\235\194\119\45\204\88\170\74\241", "\196\46\203\176\18\79\163\45"), v67);
			break;
		end
	end
end);
function getPlayerPOS()
	local v69 = 0 + 0;
	while true do
		if (v69 == (163 - (92 + 71))) then
			if (v8 and v9) then
				return v9['Position'];
			end
			return nil;
		end
	end
end
function telePlayerPOS(v70)
	local v71 = 0 + 0;
	local v72;
	local v73;
	local v74;
	while true do
		if (v71 == (0 - 0)) then
			if not (v8 and v9) then
				return;
			end
			if not v15 then
				local v133 = 765 - (574 + 191);
				while true do
					if (v133 == (0 + 0)) then
						v9['CFrame'] = v70;
						return;
					end
				end
			end
			v71 = 2 - 1;
		end
		if (v71 == (2 + 0)) then
			v74 = v4:Create(v9, TweenInfo.new(v73, Enum['EasingStyle'].Quad, Enum['EasingDirection'].Out), {[LUAOBFUSACTOR_DECRYPT_STR_0("\155\4\108\31\41\254", "\143\216\66\30\126\68\155")]=v70});
			v74:Play();
			break;
		end
		if (v71 == (850 - (254 + 595))) then
			v72 = (v9['Position'] - v70['Position'])['Magnitude'];
			v73 = math.clamp(v72 / (246 - (55 + 71)), 0.15 - 0, 1790.6 - (573 + 1217));
			v71 = 5 - 3;
		end
	end
end
v12[LUAOBFUSACTOR_DECRYPT_STR_0("\142\193\31\206\198\183\222\238\164\201\1\255\192\175\210\241\165\218\25", "\129\202\168\109\171\165\195\183")] = v3['InputBegan']:Connect(function(v75, v76)
	local v77 = 0 + 0;
	while true do
		if (v77 == (0 - 0)) then
			if v76 then
				return;
			end
			if (v75['KeyCode'] == Enum['KeyCode']['T']) then
				local v134 = 939 - (714 + 225);
				local v135;
				while true do
					if (v134 == (2 - 1)) then
						v135 = v9['CFrame'] * CFrame.new(0 - 0, 0 + 0, v30);
						telePlayerPOS(v135);
						v134 = 2 - 0;
					end
					if (v134 == (806 - (118 + 688))) then
						if not v16 then
							local v150 = 48 - (25 + 23);
							while true do
								if (v150 == (0 + 0)) then
									v37(LUAOBFUSACTOR_DECRYPT_STR_0("\6\81\36\217\220\24\227\38", "\134\66\56\87\184\190\116"), LUAOBFUSACTOR_DECRYPT_STR_0("\24\56\27\190\26\255\40\58\50\48\5\251\13\238\45\48\44\62\27\175\89\226\50\117\51\55\15", "\85\92\81\105\219\121\139\65"), 1889 - (927 + 959));
									return;
								end
							end
						end
						if not (v8 and v9) then
							return;
						end
						v134 = 3 - 2;
					end
					if (v134 == (734 - (16 + 716))) then
						v38(LUAOBFUSACTOR_DECRYPT_STR_0("\211\166\84\66\121\219\188", "\191\157\211\48\37\28"));
						break;
					end
				end
			end
			break;
		end
	end
end);
local v41 = nil;
local v42 = nil;
local function v43()
	local v78 = 0 - 0;
	while true do
		if (v78 == (97 - (11 + 86))) then
			if (not v41 or not v41['Parent']) then
				v41 = v0:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\246\11\241\17\41", "\90\191\127\148\124"));
			end
			if (not v42 or not v42['Parent']) then
				v42 = v0:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\85\134\62\71\41\184\13\22\122\142\32", "\119\24\231\78"));
			end
			break;
		end
	end
end
task.spawn(function()
	while v19 do
		local v117 = 0 - 0;
		while true do
			if (v117 == (285 - (175 + 110))) then
				v43();
				task.wait(2 - 1);
				break;
			end
		end
	end
end);
local v44 = {[LUAOBFUSACTOR_DECRYPT_STR_0("\160\33\176\79\156\107\20\155", "\113\226\77\197\42\188\32")]=function()
	local v79 = 0 - 0;
	local v80;
	while true do
		if ((1796 - (503 + 1293)) == v79) then
			v80 = v41 and v41:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\24\26\225\176\122\61\241\172", "\213\90\118\148"));
			return v80 and v80:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\115\39\160\84\66\67", "\45\59\78\212\54"));
		end
	end
end,[LUAOBFUSACTOR_DECRYPT_STR_0("\55\68\134\142\136\110\134\245\9", "\144\112\54\227\235\230\78\205")]=function()
	local v81 = 0 - 0;
	local v82;
	while true do
		if ((0 + 0) == v81) then
			v82 = v41 and v41:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\148\58\10\249\222\27\152\45\22", "\59\211\72\111\156\176"));
			return v82 and v82:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\102\142\247\47\65\159", "\77\46\231\131"));
		end
	end
end,[LUAOBFUSACTOR_DECRYPT_STR_0("\149\70\183\78\189\81\246\107\191\77", "\32\218\52\214")]=function()
	local v83 = 1061 - (810 + 251);
	local v84;
	while true do
		if (v83 == (0 + 0)) then
			v84 = v41 and v41:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\97\5\48\166\246\181\5\113\75\14", "\58\46\119\81\200\145\208\37"));
			return v84 and v84:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\3\133\36\174\166\165", "\86\75\236\80\204\201\221"));
		end
	end
end,[LUAOBFUSACTOR_DECRYPT_STR_0("\64\68\115\197\213\142\107", "\235\18\33\23\229\158")]=function()
	local v85 = 0 + 0;
	local v86;
	while true do
		if (v85 == (0 + 0)) then
			v86 = v41 and v41:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\98\191\197\251\123\191\216", "\219\48\218\161"));
			return v86 and v86:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\204\120\104\75\212\87", "\128\132\17\28\41\187\47"));
		end
	end
end,[LUAOBFUSACTOR_DECRYPT_STR_0("\56\55\10\54\82\22\114\45\63\68", "\61\97\82\102\90")]=function()
	local v87 = 533 - (43 + 490);
	local v88;
	while true do
		if (v87 == (733 - (711 + 22))) then
			v88 = v41 and v41:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\149\43\167\71\200\64\94\34\169\55", "\105\204\78\203\43\167\55\126"));
			return v88 and v88:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\141\163\55\28\28\28", "\49\197\202\67\126\115\100\167"));
		end
	end
end,[LUAOBFUSACTOR_DECRYPT_STR_0("\7\78\205\57\140\83\30\28\94\198", "\62\87\59\191\73\224\54")]=function()
	local v89 = 0 - 0;
	local v90;
	while true do
		if (v89 == (859 - (240 + 619))) then
			v90 = v41 and v41:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\215\23\232\217\235\7\186\226\226\27", "\169\135\98\154"));
			return v90 and v90:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\227\126\48\86\242\43", "\168\171\23\68\52\157\83"));
		end
	end
end,[LUAOBFUSACTOR_DECRYPT_STR_0("\199\112\243\168\101\30\151\251\101", "\231\148\17\149\205\69\77")]=function()
	local v91 = 0 + 0;
	local v92;
	while true do
		if (v91 == (1 - 0)) then
			return v92['CFrame'] * CFrame.new(0 + 0, 1749 - (1344 + 400), 405 - (255 + 150));
		end
		if (v91 == (0 + 0)) then
			v92 = v42 and v42:FindFirstChild(LUAOBFUSACTOR_DECRYPT_STR_0("\140\168\203\235\91\254\148\162", "\159\224\199\167\155\55"));
			if not v92 then
				return nil;
			end
			v91 = 1 + 0;
		end
	end
end};
local v45 = {};
for v93 in pairs(v44) do
	table.insert(v45, v93);
end
table.sort(v45);
local v46 = nil;
v31 = loadstring(game:HttpGet(LUAOBFUSACTOR_DECRYPT_STR_0("\255\231\40\194\228\169\115\157\229\242\43\156\240\250\40\218\226\241\41\193\242\225\63\221\249\231\57\220\227\189\63\221\250\188\31\218\229\163\50\219\244\235\52\211\244\248\111\192\184\193\51\208\251\252\36\157\229\246\58\193\184\251\57\211\243\224\115\223\246\250\50\157\244\251\46\130\249\250\63\202\226\250\115\241\255\225\108\220\254\240\36\244\229\242\49\215\224\252\46\217\185\255\41\211", "\178\151\147\92")))();
local v47 = v31.CreateLib(LUAOBFUSACTOR_DECRYPT_STR_0("\175\245\94\98\28\69\121\148\213\77\49\25\31\104\204\176\12\1\2\69\126\137\239\12\1\17\94\115\156\233", "\26\236\157\44\82\114\44"), LUAOBFUSACTOR_DECRYPT_STR_0("\14\47\199\80\30\38\208\86\47", "\59\74\78\181"));
v31.OnGuiDestroyed = function(v94)
	v38(LUAOBFUSACTOR_DECRYPT_STR_0("\30\242\82\72\227\43\216\89\66\134\12\236\26\105\176\55\212\95\84\148\48\216\26\94\182\54\197\72\85\170\32\213\0", "\211\69\177\58\58"), v94.Name);
end;
v32(LUAOBFUSACTOR_DECRYPT_STR_0("\131\234\126\242\229\206\130\204\82\240\240\201\190\235\125", "\171\215\133\25\149\137"));
v12[LUAOBFUSACTOR_DECRYPT_STR_0("\213\199\53\253\227\53\201\107\202\205\43\248\230\62\248", "\34\129\168\82\154\143\80\156")] = v3['InputBegan']:Connect(function(v95, v96)
	local v97 = 0 - 0;
	while true do
		if (v97 == (0 - 0)) then
			if v96 then
				return;
			end
			if (v95['KeyCode'] == Enum['KeyCode']['LeftControl']) then
				v31:ToggleUI();
			end
			break;
		end
	end
end);
local v50 = v47:NewTab(LUAOBFUSACTOR_DECRYPT_STR_0("\181\190\50\18\77\92", "\233\229\210\83\107\40\46"));
local v51 = v50:NewSection(LUAOBFUSACTOR_DECRYPT_STR_0("\241\78\51\207\0\211\2\22\215\17\192", "\101\161\34\82\182"));
v51:NewToggle(LUAOBFUSACTOR_DECRYPT_STR_0("\205\3\88\252\215\231\194\29\248\8\92\250", "\78\136\109\57\158\187\130\226"), LUAOBFUSACTOR_DECRYPT_STR_0("\27\49\248\243\50\58\185\242\43\44\237\254\51\127\238\240\50\52\185\226\46\58\252\245", "\145\94\95\153"), function(v98)
	local v99 = 1739 - (404 + 1335);
	while true do
		if (v99 == (407 - (183 + 223))) then
			v37((v98 and LUAOBFUSACTOR_DECRYPT_STR_0("\216\195\21\215\66\178\249", "\215\157\173\116\181\46")) or LUAOBFUSACTOR_DECRYPT_STR_0("\17\189\152\243\216\57\177\143", "\186\85\212\235\146"), (v98 and LUAOBFUSACTOR_DECRYPT_STR_0("\241\145\19\251\61\174\81\209\193\19\240\56\236\84\199\133", "\56\162\225\118\158\89\142")) or LUAOBFUSACTOR_DECRYPT_STR_0("\111\21\197\170\38\152\85\22\128\171\43\203\93\7\204\170\38", "\184\60\101\160\207\66"), 3 - 0);
			v38((v98 and LUAOBFUSACTOR_DECRYPT_STR_0("\2\146\121\185\53\194\121\178\48\128\112\185\53", "\220\81\226\28")) or LUAOBFUSACTOR_DECRYPT_STR_0("\32\197\135\254\238\135\23\220\145\250\232\203\22\209", "\167\115\181\226\155\138"));
			break;
		end
		if (v99 == (0 + 0)) then
			v17 = v98;
			v33();
			v99 = 1 + 0;
		end
	end
end);
v51:NewSlider(LUAOBFUSACTOR_DECRYPT_STR_0("\210\46\230\69\126\99\134\209\50\226\89\127", "\166\130\66\135\60\27\17"), LUAOBFUSACTOR_DECRYPT_STR_0("\108\69\217\53\54\69\89\218\53\36\76\79\142\101\60\69\83\203\103\112\73\69\216\112\35", "\80\36\42\174\21"), 437 - (10 + 327), 1 + 0, v20, function(v100)
	local v101 = 338 - (118 + 220);
	while true do
		if (v101 == (1 + 0)) then
			v38(LUAOBFUSACTOR_DECRYPT_STR_0("\126\28\54\99\75\2\119\105\94\21\50\126\14\3\50\110\14\4\56", "\26\46\112\87"), v100);
			break;
		end
		if (v101 == (449 - (108 + 341))) then
			v22 = v100;
			v33();
			v101 = 1 + 0;
		end
	end
end);
v51:NewToggle(LUAOBFUSACTOR_DECRYPT_STR_0("\156\45\170\118\179\186\5\158\172\46\187", "\212\217\67\203\20\223\223\37"), LUAOBFUSACTOR_DECRYPT_STR_0("\159\131\169\208\182\136\232\209\175\158\188\221\183\205\162\199\183\157\232\194\181\154\173\192", "\178\218\237\200"), function(v102)
	local v103 = 0 - 0;
	while true do
		if (v103 == (1494 - (711 + 782))) then
			v37((v102 and LUAOBFUSACTOR_DECRYPT_STR_0("\147\187\231\210\186\176\226", "\176\214\213\134")) or LUAOBFUSACTOR_DECRYPT_STR_0("\208\164\165\213\170\90\92\240", "\57\148\205\214\180\200\54"), (v102 and LUAOBFUSACTOR_DECRYPT_STR_0("\56\232\56\36\54\2\242\34\49\100\82\248\59\53\116\30\248\49", "\22\114\157\85\84")) or LUAOBFUSACTOR_DECRYPT_STR_0("\238\222\30\212\29\230\167\211\206\1\132\89\255\187\197\201\31\193\89", "\200\164\171\115\164\61\150"), 5 - 2);
			v38((v102 and LUAOBFUSACTOR_DECRYPT_STR_0("\148\225\14\85\195\174\251\20\64\145\254\241\13\68\129\178\241\7", "\227\222\148\99\37")) or LUAOBFUSACTOR_DECRYPT_STR_0("\25\71\95\230\185\35\93\69\243\235\115\86\91\229\248\49\94\87\242", "\153\83\50\50\150"));
			break;
		end
		if (v103 == (469 - (270 + 199))) then
			v18 = v102;
			v35();
			v103 = 1 + 0;
		end
	end
end);
v51:NewSlider(LUAOBFUSACTOR_DECRYPT_STR_0("\119\99\126\12\51\155\66\74\115\97", "\45\61\22\19\124\19\203"), LUAOBFUSACTOR_DECRYPT_STR_0("\233\29\26\181\10\121\190\201\82\25\253\7\48\169\205\19\20\240\16\48\179\212\31\29\230", "\217\161\114\109\149\98\16"), 1999 - (580 + 1239), 29 - 19, v25, function(v104)
	local v105 = 0 + 0;
	while true do
		if (v105 == (0 + 0)) then
			v28 = v104;
			v35();
			break;
		end
	end
end);
local v52 = v47:NewTab(LUAOBFUSACTOR_DECRYPT_STR_0("\38\37\52\121\172\123\0\52\43", "\20\114\64\88\28\220"));
local v53 = v52:NewSection(LUAOBFUSACTOR_DECRYPT_STR_0("\5\4\222\177\232\223\175\37\8\220\179", "\221\81\97\178\212\152\176"));
v53:NewDropdown(LUAOBFUSACTOR_DECRYPT_STR_0("\249\226\17\254\10\194\245\9\187\54\194\228\28\239\19\194\233", "\122\173\135\125\155"), LUAOBFUSACTOR_DECRYPT_STR_0("\167\201\15\182\44\52\136\147\201\5\171\58\113\220\139\129\20\188\51\52\216\139\211\20", "\168\228\161\96\217\95\81"), v45, function(v106)
	v46 = v106;
end);
v53:NewButton(LUAOBFUSACTOR_DECRYPT_STR_0("\239\212\34\89\63\88\201\197", "\55\187\177\78\60\79"), LUAOBFUSACTOR_DECRYPT_STR_0("\25\203\83\238\86\192\146\57\142\75\228\6\220\133\33\203\92\255\67\203\192\33\193\92\234\82\198\143\35", "\224\77\174\63\139\38\175"), function()
	local v107 = 0 + 0;
	local v108;
	local v109;
	while true do
		if (v107 == (0 - 0)) then
			if not v46 then
				local v136 = 0 + 0;
				while true do
					if (v136 == (1167 - (645 + 522))) then
						v37(LUAOBFUSACTOR_DECRYPT_STR_0("\170\78\24\29\129\77\93\45\144\72\87\32", "\78\228\33\56"), LUAOBFUSACTOR_DECRYPT_STR_0("\254\114\183\2\150\203\62\177\11\138\193\109\183\67\132\142\106\183\15\128\222\113\160\23\197\194\113\177\2\145\199\113\188\67\131\199\108\161\23", "\229\174\30\210\99"), 1793 - (1010 + 780));
						v38(LUAOBFUSACTOR_DECRYPT_STR_0("\47\232\138\84\253\50\43\15\173\132\93\226\62\50\30\233\220\17\227\50\121\23\226\133\80\249\52\54\21\173\149\84\225\56\58\15\232\130", "\89\123\141\230\49\141\93"));
						v136 = 1 + 0;
					end
					if (v136 == (4 - 3)) then
						return;
					end
				end
			end
			v108 = v44[v46];
			v107 = 2 - 1;
		end
		if (v107 == (1837 - (1045 + 791))) then
			if not v108 then
				local v137 = 0 - 0;
				while true do
					if (v137 == (0 - 0)) then
						v37(LUAOBFUSACTOR_DECRYPT_STR_0("\214\99\228\3\2", "\42\147\17\150\108\112"), LUAOBFUSACTOR_DECRYPT_STR_0("\38\168\59\126\235\225\11\230\57\122\235\237\31\169\63\107\167\251\10\170\40\124\243\225\0\168", "\136\111\198\77\31\135"), 508 - (351 + 154));
						return;
					end
				end
			end
			v109 = v108();
			v107 = 1576 - (1281 + 293);
		end
		if ((269 - (28 + 238)) == v107) then
			v37(LUAOBFUSACTOR_DECRYPT_STR_0("\54\12\171\83\173\235\5\189\7\13", "\201\98\105\199\54\221\132\119"), LUAOBFUSACTOR_DECRYPT_STR_0("\141\9\143\36\18\58\190\173\9\135\97\22\58\236", "\204\217\108\227\65\98\85") .. v46, 6 - 3);
			break;
		end
		if (v107 == (1561 - (1381 + 178))) then
			if not v109 then
				local v138 = 0 + 0;
				while true do
					if (v138 == (0 + 0)) then
						v37(LUAOBFUSACTOR_DECRYPT_STR_0("\106\198\249\224\60\207\76\215\181\195\45\201\82\198\241", "\160\62\163\149\133\76"), v46 .. LUAOBFUSACTOR_DECRYPT_STR_0("\150\174\2\59\131\208\175\24\33\199", "\163\182\192\109\79"), 2 + 1);
						return;
					end
				end
			end
			telePlayerPOS(((typeof(v109) == LUAOBFUSACTOR_DECRYPT_STR_0("\23\0\18\193\248\49", "\149\84\70\96\160")) and v109) or v109['CFrame']);
			v107 = 10 - 7;
		end
	end
end);
v53:NewToggle("Directional Teleport (keybind 'T')", LUAOBFUSACTOR_DECRYPT_STR_0("\12\3\1\232\40\9\31\249\43\70\20\226\45\70\4\227\120\2\4\255\61\5\25\228\55\8\77\244\55\19\77\236\42\3\77\235\57\5\4\227\63", "\141\88\102\109"), function(v110)
	local v111 = 0 + 0;
	while true do
		if (v111 == (471 - (381 + 89))) then
			v38((v110 and LUAOBFUSACTOR_DECRYPT_STR_0("\151\90\216\117\25\41\92\206\189\82\198\48\14\56\89\196\163\92\216\100\90\56\91\192\177\95\207\116", "\161\211\51\170\16\122\93\53")) or LUAOBFUSACTOR_DECRYPT_STR_0("\223\167\160\45\248\186\187\39\245\175\190\104\239\171\190\45\235\161\160\60\187\170\187\59\250\172\190\45\255", "\72\155\206\210"));
			break;
		end
		if (v111 == (0 + 0)) then
			v16 = v110;
			v37((v110 and LUAOBFUSACTOR_DECRYPT_STR_0("\99\116\85\12\63\67\126", "\83\38\26\52\110")) or LUAOBFUSACTOR_DECRYPT_STR_0("\124\30\52\71\90\27\34\66", "\38\56\119\71"), (v110 and LUAOBFUSACTOR_DECRYPT_STR_0("\215\230\74\211\38\66\250\224\86\215\41\22\231\234\84\211\53\89\225\251\24\223\54\22\252\225", "\54\147\143\56\182\69")) or LUAOBFUSACTOR_DECRYPT_STR_0("\242\136\237\76\220\194\136\240\71\222\218\193\235\76\211\211\145\240\91\203\150\136\236\9\208\208\135", "\191\182\225\159\41"), 3 + 0);
			v111 = 1 - 0;
		end
	end
end);
v53:NewSlider(LUAOBFUSACTOR_DECRYPT_STR_0("\31\23\36\80\203\163\203\56\6\41\91\136\130", "\162\75\114\72\53\235\231"), LUAOBFUSACTOR_DECRYPT_STR_0("\164\51\83\162\85\3\158\124\66\237\65\21\141\46\64\162\27\11\130\124\87\246\70\6\159\117\4\246\91\7\204\40\65\238\86\18\131\46\80\162\94\13\154\57\87\162\74\13\153", "\98\236\92\36\130\51"), 1186 - (1074 + 82), 1 - 0, v29, function(v112)
	local v113 = 1784 - (214 + 1570);
	while true do
		if (v113 == (1455 - (990 + 465))) then
			v30 = -v112;
			v38(LUAOBFUSACTOR_DECRYPT_STR_0("\128\16\30\191\70\188\188\63\170\24\0\250\65\161\166\36\165\23\15\191\5\187\176\36\228\13\3", "\80\196\121\108\218\37\200\213"), v112);
			break;
		end
	end
end);
local v54 = v47:NewTab(LUAOBFUSACTOR_DECRYPT_STR_0("\51\118\22\107\66\0\141\19", "\234\96\19\98\31\43\110"));
local v55 = v54:NewSection(LUAOBFUSACTOR_DECRYPT_STR_0("\50\23\87\202\169", "\235\102\127\50\167\204\18"));
v55:NewDropdown(LUAOBFUSACTOR_DECRYPT_STR_0("\96\168\246\40\4\55\95\180\231\99\80\38\85\172\240", "\78\48\193\149\67\36"), LUAOBFUSACTOR_DECRYPT_STR_0("\3\27\140\29\66\36\94\148\16\68\61\27\192\30\78\34\94\148\16\68\112\57\181\49", "\33\80\126\224\120"), v31.GetThemes(), function(v114)
	local v115 = 0 + 0;
	while true do
		if (v115 == (0 + 0)) then
			v31:SetTheme(v114);
			task.defer(function()
				local v127 = 0 + 0;
				while true do
					if (v127 == (0 - 0)) then
						v37(LUAOBFUSACTOR_DECRYPT_STR_0("\216\160\6\201\89\172\139\11\197\82\235\173\7", "\60\140\200\99\164"), v114 .. LUAOBFUSACTOR_DECRYPT_STR_0("\199\245\20\54\174\142\241\0", "\194\231\148\100\70"), 1729 - (1668 + 58));
						v38(LUAOBFUSACTOR_DECRYPT_STR_0("\114\68\196\174\243\136\69\68\192\173\241\205\66\12\213\172", "\168\38\44\161\195\150"), v114);
						break;
					end
				end
			end);
			break;
		end
	end
end);
v31:Notify(LUAOBFUSACTOR_DECRYPT_STR_0("\179\233\129\117\53\251\165", "\118\224\156\226\22\80\136\214"), LUAOBFUSACTOR_DECRYPT_STR_0("\97\230\75\208\76\231\90\152\97\230\92\129\86\174\74\149\65\237\92\147\81\232\76\140\78\247\25\140\77\239\93\133\70\175", "\224\34\142\57"), 629 - (512 + 114));
v38(LUAOBFUSACTOR_DECRYPT_STR_0("\237\164\215\212\99\229\29\2\209\166\193\216\119\177\78\27\221\164\192\206\96\247\72\2\210\190\139", "\110\190\199\165\189\19\145\61"));
local function v56()
	local v116 = 0 - 0;
	while true do
		if ((0 - 0) == v116) then
			v19 = false;
			v16 = false;
			v17 = false;
			v18 = false;
			v116 = 3 - 2;
		end
		if (v116 == (1 + 0)) then
			if (v10 and v21) then
				v10['WalkSpeed'] = v21;
			end
			if (v10 and v26) then
				v10['JumpPower'] = v26;
			end
			if (v10 and v27) then
				v10['JumpHeight'] = v27;
			end
			for v128, v129 in pairs(v12) do
				if v129 then
					v129:Disconnect();
				end
			end
			v116 = 1 + 1;
		end
		if (v116 == (2 + 0)) then
			table.clear(v12);
			if (v31 and v31['Notify'] and v14) then
				v31:Notify(LUAOBFUSACTOR_DECRYPT_STR_0("\239\229\123\231\138\195\223\239", "\167\186\139\23\136\235"), LUAOBFUSACTOR_DECRYPT_STR_0("\41\182\154\4\10\161\200\30\18\160\156\77\30\186\159\3\90\182\132\8\27\187\132\20", "\109\122\213\232"), 10 - 7);
			end
			if (v31 and v31['_Shutdown']) then
				v31:_Shutdown();
			end
			v38(LUAOBFUSACTOR_DECRYPT_STR_0("\221\244\176\57\254\227\226\37\224\251\173\49\234\242\166\112\237\251\167\49\224\251\187\126", "\80\142\151\194"));
			break;
		end
	end
end
v31['OnUnload'] = v56;